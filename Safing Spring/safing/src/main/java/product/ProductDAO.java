...
public class ProductDAO {
	...
	//장바구니 담기 상품
	public int insert_cart_pack(List<CartVO> list){
		int result = 0;
		ArrayList<Integer> results = new ArrayList<Integer>();
		results.add(sql.insert("product.mapper.insert_cart_pro", list.get(0)));
		
		for (CartVO vo : list) {
			results.add(sql.insert("product.mapper.insert_cart_pack", vo));
		}
	...
	//리뷰 좋아요 수정
	public void board_like_cnt_update(List<Integer> list) {
		sql.update("product.mapper.board_like_cnt_update", list);
	}
