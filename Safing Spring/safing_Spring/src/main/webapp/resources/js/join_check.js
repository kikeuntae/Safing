var join = {
	...
	//인지를 구분(판단)하여 유효성 검사 진행하고자 함
		if(tag == 'member_id') return this.id_status(data);	
		else if(tag == 'member_pw') return this.pw_status(data);	
	 	else if(tag == 'member_pw_ck') return this.pw_ck_status(data);	
	}
	...
	, pw_ck_status : function(pw_ck){
		...
		else if(pw_ck == $('[name=member_pw]').val()) return this.pass.equal;
		
		else if(pw_ck != $('[name=member_pw]').val()) return this.pass.notequal;
	...
	, common : { // 공통적으로 검사를 해야하는 항목 지정
		empty :{code : 'invalid', desc : '정보를 입력하세요'}
