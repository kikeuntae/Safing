		...
		<tr>
			...
			<td>
				<div class="manage-image">
					...
					<a  data-toggle="modal" data-target="#member_delete${vo.member_id}">
...
<!-- detail Modal  delete-->
 <c:forEach var="vo" items="${list}">
<div class="modal fade" id="member_delete${vo.member_id}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog w-40">
    <div class="modal-content">
     <div class="text-black text-center">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">회원정보 삭제</h5>
        <button type="button" class="close border-none" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
       <form name="member_delete${vo.member_id}" action="member_delete.cu" method="post">
       <input type="hidden" name='member_id' value="${vo.member_id}">
		   <table class="table w-100 my-sm-1 table-center">
			  <tbody>
			    ...
			    <tr>
			       ...
			       <td class="text-align text-left">${vo.member_id} 회원님을 삭제하시겠습니까?</td>
			    </tr>   
      ...
      <div class="modal-footer">
      	<div class="text-center btn-center">
       	 	<button type="button" class="btn btn-secondary mx-2" data-dismiss="modal" onclick="$('[name=member_delete${vo.member_id}]').submit()">삭제</button>
       	 	<button type="button" class="btn btn-outline-success mx-2" data-dismiss="modal">닫기</button>
      	</div>
      </div>
    </div>
