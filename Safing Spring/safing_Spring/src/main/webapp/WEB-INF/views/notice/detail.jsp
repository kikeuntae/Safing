...
<div class="text-center my-sm-3 py-4">
	...
	<c:if test="${(vo.member_id eq loginInfo.member_id ) or (loginInfo.member_admin eq 'y')   }">
		<button type="button" class='btn btn-outline-success mx-2' onclick='$("form").attr("action", "modify.no"); $("form").submit()' >수정</button>
		
		<button type="button" class='btn btn-outline-delete mx-2' onclick="notice_delete(${vo.board_id})" >삭제</button>
	</c:if>
</div>
...
function notice_delete(board_id){
	swal({
		...
		.then((willDelete) => {
		  if (willDelete) {
			  location.href="notice_delete.no?board_id="+board_id;
		  } else {
			return false;
		  }
	});
